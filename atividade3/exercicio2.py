    ...
    if x != None:
        for y in grafo.vizinhosSaida(x):
            ...
            if distancia[mate[y]] == distancia[x] + 1:
                if DFS(grafo, mate, mate[y], distancia) == True:
                    mate[y] = x
                    mate[x] = y
                    return True
        distancia[x] = sys.float_info.max
...
def hopcroft_karp(grafo):
    ...
    m = 0
    while BFS(grafo, mate, distancia, X) == True:
        for x in X:
            if mate[x] == None:
                if DFS(grafo, mate, x, distancia) == True:
                    m += 1
...
def main():
    grafo = Grafo.Grafo("testes/emparelhamento_maximo/pequeno.net")
    hopcroft_karp(grafo)
